        ...
        match self.content {
            VariableContent::Referenceable(referenceable) => {
                let inner = MutableSubRcRefCell::new(referenceable)
                    .map_err(|_| self.variable_span.ownership_error::<FunctionError>(MUTABLE_ERROR_MESSAGE))?;
                Ok(Mutable(inner))
            }
        ...
        match self.content {
            VariableContent::Referenceable(referenceable) => {
                let inner = SharedSubRcRefCell::new(referenceable)
                    .map_err(|_| self.variable_span.ownership_error::<FunctionError>(SHARED_ERROR_MESSAGE))?;
                Ok(Shared(inner))
            }
                ...
                match MutableSubRcRefCell::new(referenceable) {
                    ...
                    Err(referenceable) => {
                        let shared = SharedSubRcRefCell::new(referenceable).map_err(|_| {
                            self.variable_span.ownership_error::<FunctionError>(SHARED_ERROR_MESSAGE)
                        })?;
                        Ok(LateBoundValue::Shared(LateBoundSharedValue::new(
                            Shared(shared),
